...
#reslut1 = pool.apply_async(dupChecker, ['output5.txt','txt/list5kb.txt'])
#reslut1 = pool.apply_async(dupChecker, ['output6.txt','txt/list6kb.txt'])
#x = p.map(dupChecker, ['output5.txt','txt/list5kb.txt'])
#y = p.map(dupChecker, ['output6.txt','txt/list6kb.txt'])
def dupChecker(input1):
    stdout = open('output6.txt','a')
    stdin = open(input1,'r')
    lines = stdin.readlines()
    match = False
    for x in range(0,len(lines)-1):
            original = cv2.imread(lines[x].rstrip(),1)
            # takes a random saple of 100 photos to compare agaist the base
            for z in range(100):
...
def compare_images(imageA, imageB, title,x,y,stdout,lines):
        ...
        if(m<=500):
                ...
                if(s >= .8):
                        ...
                        stdout.write("\n")
                        print(lines[x].rstrip() + " vs " + lines[y+1].rstrip()+ ", ")
...
if __name__ == '__main__':
    with Pool(5) as p:
        print(p.map(dupChecker, ['txt/list5kb.txt', 'txt/list6kb.txt']))
